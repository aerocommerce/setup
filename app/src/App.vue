        ...
        <aside class="relative xl:flex-grow p-6 xl:order-2 overflow-hidden">
          ...
          <transition name="fade">
            <div
                ...
                :style="{
                        transform:
                            'translateX(calc(-33vh * ' +
                            currentStepEntry.illustrationShift +
                            ')) translateY(-50%)',
                    }"
                :class="currentStepEntry.hasOwnProperty('subComponent') ? 'duration-150 delay-1000' : 'duration-1000 delay-150'"
            >
              ...
              >
                <transition name="fade">
                  ...
                  >
                    ...
                    <span
                        class="text-4xl"
                        :class="setupData.agora && currentStep === getStepKeyIndex('project') + 1 ? 'animation-wave' : ''"
                    >👋</span
                    >
                  </div>
                </transition>
                ...
                <div class="flex flex-wrap flex-col xl:flex-row gap-6 justify-between xl:items-center">
                  ...
                  <ol class="flex flex-wrap gap-3 items-center text-alpha">
                    <li v-for="(step, stepIndex) in steps" class="flex items-center">
                      <button
                          @click.prevent="!stepsComplete && currentStep > stepIndex ? jumpToStep(stepIndex) : null"
          ...
          <transition name="delayFade">
            <component v-if="currentStepEntry && currentStepEntry.hasOwnProperty('subComponent')"
...
<script>
    ...
    const currentStepEntry = computed(() => steps.value.hasOwnProperty(currentStep.value)
