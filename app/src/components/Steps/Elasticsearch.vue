<template>
    <Step>
        <template #title> Elasticsearch connection </template>

        <template #description>
            Elasticsearch is used to index your product listings in order for the data to be rapidly searchable,
            sortable and filterable. You can choose to connect to a local or remote instance depending on your setup.
        </template>
        ...
        <form @submit.prevent="attemptAdvance" method="post">
            <div class="mb-9">
                <ContentGroup
                    id="local-es"
                    v-model="setupData.project.elasticsearchConnectionType"
                    name="elasticsearchConnectionType"
                    value="local"
                >
                    <template #title> Local Elasticsearch connection </template>
                    ...
                    <div class="mb-6 flex items-center">
                        <div class="w-full">
                            ...
                            <input
                                type="text"
                                id="local-es-host-name"
                                placeholder="Enter host name"
                                v-model="setupData.project.elasticsearchHost"
                            />
                        </div>
                        ...
                        <div class="">
                            ...
                            <input
                                type="number"
                                id="local-es-port-number"
                                placeholder="Enter port no"
                                autocomplete="off"
                                v-model="setupData.project.elasticsearchPort"
                            />
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        <SingleAccordion v-model="setupData.project.showAdvancedElasticsearchOptions">
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    id="local-es-store-id"
                                    placeholder="Enter store identifier"
                                    autocomplete="off"
                                    v-model="setupData.project.storeIdentifier"
                                />
                            </div>
                        </SingleAccordion>
                    </div>
                    ...
                    <button v-else class="button button-secondary" type="button" @click="testConnection">
                        <RefreshIcon
                            ...
                            :class="{ 'animation-rotate': testingEs }"
                        />
                        Test connection
                    </button>
                </ContentGroup>
            ...
            <div>
                <ContentGroup
                    ...
                    value="remote"
                >
                    <template #title> Remote Elasticsearch connection </template>
