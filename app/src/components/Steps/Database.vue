<template>
    <Step>
        ...
        <template #description>
            Create or select the database you want to use for this store. We will create the tables needed if they don't
            already exist and scan for an existing catalog, theme and user account.
        </template>
        ...
        <form @submit.prevent="attemptAdvance" method="post">
            <div class="mb-9">
                <ContentGroup
                    id="new-db"
                    v-model="setupData.project.databaseType"
                    group="databaseType"
                    value="new_database"
                >
                    <template #title> Create new database </template>
                    ...
                    <div class="mb-6">
                        ...
                        <input
                            ...
                            autocomplete="off"
                            v-model="setupData.project.database"
                        />
                    </div>
                </ContentGroup>
            </div>
            ...
            <div>
                ...
                >
                    <template #title> Select existing database </template>

                    <WarningMessage v-if="matchedDatabase" class="mb-6">
                        A database already exists that we think you may wish to use. We've selected it for you below,
                        however you can choose a different name or create a new database.
                    </WarningMessage>

                    <div class="mb-6">
                        <select id="exisiting-db" v-model="setupData.project.database">
                            <option value="" disabled>Select an existing database</option>
                            <option v-for="database in setupData.project.databases" :value="database">
                                {{ database }}
                            </option>
                        </select>
                    </div>
        ...
        setup() {
            ...
            const attemptAdvance = () => {
                ...
                if (!setupData.project.database.length) {
                    ...
                    let name = setupData.project.database.toLowerCase().replace(/[a-z0-9_-]/g, '')
